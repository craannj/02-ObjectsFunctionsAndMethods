...
def main():
    ...
    even_better_draw_circles(rg.Point(100, 50), 4, 10, 'red', 5)
    even_better_draw_circles(rg.Point(-200, 0), 6, 40, 'blue', 10)
    window.update()
...
def even_better_draw_circles(point, a, n, color, t):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen = rg.Pen(color, t)
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(n):  # k becomes 1, 2, 3, ... 10
